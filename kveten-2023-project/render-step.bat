...
rem "D:\Program Files\MeGUI\tools\ffmpeg\ffmpeg.exe" -hide_banner -hwaccel qsv -hwaccel_output_format qsv -init_hw_device qsv:hw,child_device_type=dxva2 -i "%2.avs" -c:v hevc_qsv -preset veryslow -global_quality 27 -y "%1\result\%2.h265"
    "C:\Users\ilx\wrk\ffmpeg\ffmpeg.exe"             -hide_banner -hwaccel dxva2                                                                         -i "%2.avs" -c:v hevc_amf -quality 0 -level 6 -rc 0 -qp_i 28 -qp_p 28 -y "%1\result\%2.h265"
...
mkdir "%dir_f%"
rem "D:\Program Files\MeGUI\tools\mp4box\mp4box.exe" -add "%1\result\%2.h265#trackID=1:fps=60.0:par=1:1:name=" -add "%1\result\%2.ac3#trackID=1:name=" -tmp "%dir_f_ds%" -new "%1\result\540\%2.mp4"
